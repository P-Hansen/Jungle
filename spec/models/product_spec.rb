...
RSpec.describe Product, type: :model do
  describe 'Validations' do
    ...
    xit "can create a new category with the right name" do
      expect(cat.name).to eq "testing"
    end

    xit "can create a new product with a name" do
      expect(prod.name).to eq "that book"
    end

    xit "can create a new product with a description" do
      expect(prod.description).to eq "you know the one"
    end

    xit "can create a new product with a price" do
      expect(prod.price).to eq 60
    end

    xit "can create a new product with a quantity" do
      expect(prod.quantity).to eq 15
    end

    xit "can create a new product with a category" do
